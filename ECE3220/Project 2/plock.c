 ...
 * Project 2
 * 
 * Assumptions: to use the mutex lock and condition variables to do the proceed threads
 * with different priorities and first come and first serve. So basicly the threads will have
 * to arrive based on how they come and how they proeceed
 * 
 * Bugs: Unfortunately, I can not test through the case that instructor gave me, because the
 * threads comes to the lock can not prevent the newly coming thread that comes to enqueue early.
 * 
 */
#include <stdio.h>
#include <stdlib.h>
 ...
 void plock_exit(plock_t* lock)
{
	...
	pthread_cond_signal(&(DeQ_node->waitCV));
