...
ENTITY airplane_vhd_tst_1 IS
END airplane_vhd_tst_1 ;
ARCHITECTURE airplane_arch OF airplane_vhd_tst_1 IS
-- constants                                                 
-- signals                                                   
...
BEGIN 
   ...
   reset <= '1';                 -- test for the asynchronus interrupt from the reset, and nothing else changed from the orignal code
	
   wait for 10ns; 
	reset <= '0'; wait for 3ns; 	
   reset <= '1';                 -- test for the asynchronus interrupt from the reset, and nothing else changed from the orignal code
	wait for 2ns;
	alt_10k <= '0';  -- assume the plane will go beyond the 25k feet
	wait for 10ns; alt_25k <= '1';  -- but if the alt_25k goes high in that situation, the current stage won't change
