 ...
 *
 * Assumptions: Same as MP2, just mentioned below.
 *
...
/* Function to create a list using list_insert() 
*/
void task_list_add_tail_mp3(list_t * list_ptr, int priority)
{
    ...
    list_insert(list_ptr, rec_ptr, num_in_list);
    num_in_list = 0;
...
void task_list_stats(list_t * sorted, list_t * unsorted, list_t* mp3)
{
    ...
    int num_in_mp3_sorted_list = list_order(mp3);
    char* sort_order_mp3;
...
{
    task_t *rec_ptr = (task_t*) malloc(sizeof(task_t));  // fixed
    ...
    if (num_record == max_list_size) {
        added_return = -1; //list is full
    } else if (num_record < max_list_size) {
        list_insert_sorted(list_ptr, rec_ptr);
        added_return = 1; //insert successfully
    } else {
        added_return = 0; //unexpected consequences
    } 
...
{
    task_t *rec_ptr = list_access(list_ptr, LISTPOS_HEAD); //fixed
