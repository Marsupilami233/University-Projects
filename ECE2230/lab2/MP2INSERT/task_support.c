/* task_support.c         <<-- A template completed
 * Stud. Yulin Xiao          <<-- many updates finished
 * Yulinx
 ...
 * Assumptions: 
 *
 * Bugs: None that I know, except adding the negetive priority will generate the assert error
 * But the Instructors code can not be changed as mentioned.
 *
...
void task_list_print(list_t * list_ptr, char *list_type)
{
    ...
    if (num_in_list == 0) {
        ...
        for (i = 0; i < num_in_list; i++) {
            printf("%4d: ", i);
            rec_ptr = list_access(list_ptr, i); //<--fixed to grab record at pos i!
...
void task_list_add(list_t * list_ptr, int max_list_size)
{
    ...
    int num_record = list_size(list_ptr); 
    rec_ptr = (task_t*) calloc(1,sizeof(task_t));  
    ...
    if (num_record == max_list_size) {
        added_return = -1;
    } else if (num_record < max_list_size) {
 ...
 * list.  
 * Inputs: task_id and the list pointer list_ptr
 * Return: No return values
 * Output: Found task_id and current pos_index
 * Variable: i is loop variable, to approach the pos_id: index, num_record is the number of list 
 */
void task_list_lookup_id(list_t * list_ptr, int id)
{
    ...
    int i;
    int num_record = list_size(list_ptr);
    task_t *rec_ptr = NULL;                //<--first to set NULL
    task_t *task = NULL;
    for (i = 0; i < num_record; i ++) {
        task = list_access(list_ptr, i);   // traverse the list to find match id
        if (task->task_id == id) {
            rec_ptr = task;
            index = i;
        }
 ...
 * task id
 * Input: task_id and list pointer
 * Output: NULL
 * Return: NULL
 * Variable: i is loop variable, num_records is the list size, task to take the pos_index.
 * Using the rec_ptr to return the tasks that are to remove and move the node and clean the
 * content.
 */
void task_list_remove_id(list_t * list_ptr, int id)
{
    
    task_t *rec_ptr = NULL;   // TODO: fixed<--------------
    int i = 0;
    task_t *task = NULL;      // used to find the task id<-------
    int num_records;
    num_records = list_size(list_ptr);
    for (i = 0; i < num_records; i ++) {
        task = list_access(list_ptr, i); // go throught the taskes with pos_index
       
        if (task->task_id == id) {
            
            rec_ptr = list_access(list_ptr, i);// i is the pos_index    
...
void task_list_update(list_t * list_ptr, int id, int state)
{
    ...
    int pos_index = 0; // the position index do not messed up with the task_id
    int num_in_list = 0;
    num_in_list = list_size(list_ptr);
    
    while(pos_index < num_in_list) {
        rec_ptr = list_access(list_ptr, pos_index);
        
            if (rec_ptr->task_id == id && rec_ptr != NULL) {
                rec_ptr->state = state; //update state
                break;
            } else {
                rec_ptr = NULL;
            }
    ...
    rec_ptr = NULL; //set to NULL top prevent the unexcepted access
} 


/* This function schedules based on id and priority 
 * always schedule the id first, if its valid. And if not found any of valid id, then schedule priority 
 */
void task_list_schedule(list_t * list_ptr, int id, int priority)
{
    ...
    /* TODO:always schedule the id first, if its valid, then schedule the priority */
    for (i = 0; i < num_in_list; i ++) {
        rec_ptr = list_access(list_ptr, id); // accept the return value!!
        if ((rec_ptr->task_id == id) && (rec_ptr->state == QUEUED || rec_ptr->state == BLOCKED )) {
            rec_ptr->state = RUNNING;
            printf("Now running task:%d\n", rec_ptr->task_id);
            print_task_rec_short(rec_ptr);
            
        }
    }
    /*schedule the priority first, but notice that the priority must be positive*/
    for (i = 0; i < num_in_list; i ++) {
        rec_ptr = list_access(list_ptr, id);
        if ((rec_ptr->priority == priority) && (rec_ptr->state == QUEUED || rec_ptr->state == BLOCKED)) {
            rec_ptr->state = RUNNING;
            printf("Now running task:%d\n", rec_ptr->task_id);
            print_task_rec_short(rec_ptr);
        }
...
void task_list_determine(list_t * list_ptr)
{
    ...
    int num_records = list_size(list_ptr);
    int i,j;
    
    task_t *task = NULL;
    for (i = 0; i < num_records; i ++) { // i is used to traverse the num_records, j is used to compare the arguments.
        task = list_access(list_ptr, i);
        if ((task->state == QUEUED) || (task->state == BLOCKED)) {
            //once find the runnable tasks, then break
            for (j = 0; j < num_args; j ++) {
                if (task->args[j] != args[j]) {
                    break;
                } else if (j == num_args - 1) {
                    idx = i; // Again, replace the idx with the loop variable
                }
            }
        }    
    }
    ...
    free(args); //args is allocated in getDetermineArgs()
}
/* This function removes the finished task from the list without changing 
 * the index of the and removes to the front
 */
void task_list_remove_finished(list_t * list_ptr)
{
    task_t* task = NULL;
    //int if_finished = 0 // flag to show that the task have already finished
    list_t* rm = NULL;  
    /*TODO: Find tasks That Are Removeable(FINISHED)*/
    int idx, rm_N = 0;
    rm = list_construct(task_compare, task_rec_cleanup);
    int num_of_records = list_size(list_ptr);// number of the list_size
    for (idx = 0; idx < num_of_records; idx ++) {
        task = list_access(list_ptr, idx);
        if (task->state == FINISHED && task !=NULL) {
            task = list_remove(list_ptr, idx);
            list_insert_sorted(rm, task); // if not the queue, insert into a sorted list rm            
        }
    }  

    /*TODO: Remove the task*/
    /*Remember that you should move the list_node_t to the front and cover the blank*/
    
    int num_in_list = list_size(rm);
    printf("Removing %d finshed tasks\n", num_in_list);
    for (rm_N = 0; rm_N<num_in_list; rm_N++) {
    
        task = list_access(rm, rm_N);
        print_task_rec_short(task);
        
        //task_rec_cleanup(task); //if clean up, it will result in double free
...
{
    task_t *rec_ptr = (task_t *) calloc (1,sizeof(task_t));  // fixed
...
{
    task_t *task_ptr = list_remove(list_ptr, LISTPOS_HEAD);// fixed
...
{
    task_t *rec_ptr = list_access(list_ptr, LISTPOS_HEAD); 
